...
let over80std = students.filter(students => students.score >= 80).map(students => students.name);
console.log("80 or more:", over80std);
...
// TODO
let rank = students.sort((a,b) => b.score - a.score).slice(0,3);
for(let i = 0;i<3;i++){
  console.log(i+1,".",rank[i].name,"(",rank[i].dept,")",rank[i].score);
}